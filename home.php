...
if(isset($_GET['zip'])) {
  ...
  chdir("c:/www/rpf/html/tu/".$zip_dir);
  $dir = opendir(".");
  $bytes=0;
...
// reset bad session after manaul CAPTCHA pass
if(isset($_GET['reset'])) {
	$rs=$_GET['reset'];
	$bad_sess_file="bad_session_".$rs.".txt";
...
while($info = $check->fetch_assoc()) {
	...
	if($info['completed']) $actions="<a href='home.php?zip=".$info['id']."'><img src='zip.png' border=0 title='create and download zip file of completed job'></a>";
	$dl="";
	if(file_exists("JOB_".$info['id']."/JOB_".$info['id'].".zip")) {
		$dl="&nbsp;<a href='JOB_".$info['id']."/JOB_".$info['id'].".zip'><img src='download.png' title='download large zip file' border=0></a>";
	}
	echo "<tr>";
	echo "<td align='center'>".$actions."&nbsp;&nbsp;".$dl."</td>";
